# 开发规范
本文只描述“如何添加”，具体细节请参考对应层级的说明文档。

## 新建用户
1. 在 `users/<name>/profile.nix` 写 profile，至少包含 `user.username`，
...
## 新建主机
1. 新建目录 `hosts/<hostname>`，包含 `profile.nix` 与
 `hardware-configuration.nix`。
2. `profile.nix` 中设置 `host.hostname`（与目录名一致）、
`host.system`、`host.users`，并按需配置 `desktop` 与 `system.software`。
